...
function SignIn() {
  ...
  useEffect(() => {
    setAuthText(
      <>
        <h3>
          효율적인 일정 관리의 <span>새로운 시작</span>
        </h3>
        <p>
          Timely와 함께 팀의 생산성을 높이고, 프로젝트를 체계적으로 관리하세요.
          더 스마트한 협업이 시작됩니다.
        </p>
        <ul>
          <li>
  ...
  return (
    <>
        ...
        <fieldset>
          ...
          <S.SinInOptions>
            <CheckboxField
              text="로그인 상태 유지"
              id="loginState"
              checkBoxState={loginForm.loginState}
            >
              <Checkbox
                type="checkbox"
                checked={loginForm.loginState}
                onChange={() => {
      ...
      <S.SinUpText>
        계정이 없으신가요?{" "}
        <S.SignUpLink to="/auth/signup">회원가입</S.SignUpLink>
      </S.SinUpText>
      <S.Links>
        <S.LinkList>
          <Link to="">아이디찾기</Link>
